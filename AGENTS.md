...
- Real design experiments must run the full implemented reputation pipeline. Do not silently disable bonus, leader recovery, PeerTrust, strong-fault detectors, or certified weight updates to make one experiment look cleaner. If a component is intentionally disabled, label the run as an ablation or diagnostic control, not as a main-system result.
