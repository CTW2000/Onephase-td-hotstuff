#!/bin/bash
# Scalability experiment for n=2, 10, 20 — all 4 protocols, sequential.
# Uniform clientBatchNum=100 for fair cross-protocol comparison.
...
RESULT_DIR="$DEPLOY_DIR/experiment_results/scalability_n2_10_20"
mkdir -p "$RESULT_DIR"
...
echo ""
echo "######################################################################"
echo "# SCALABILITY EXPERIMENT | n=${NS[@]} | batch=100"
echo "######################################################################"
...
"
    # Verify config wrote successfully (catches silent permission failures)
    actual_mpt=$(grep max_process_txn ./config/${cfg}.config | grep -o '[0-9]\+')
    if [ "$actual_mpt" != "$mpt" ]; then
      echo "ERROR: ${cfg}.config has max_process_txn=$actual_mpt, expected $mpt — ABORTING"
      exit 1
    fi

    result_file="$RESULT_DIR/${proto}_n${n}.txt"
...
echo ""
echo "======================================================================"
echo "  SCALABILITY EXPERIMENTS COMPLETE (n=${NS[@]})"
