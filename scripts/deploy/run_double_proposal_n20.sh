#!/bin/bash
# TD-Hotstuff double-proposal strong-fault experiment for n=20.
# TD_HS_DOUBLE_PROPOSAL_IDS is used only by deploy_multi.sh to inject the
# replica-local TD_HS_DOUBLE_PROPOSAL=1 flag into selected faulty processes.
...
RESULT_DIR="$DEPLOY_DIR/experiment_results/double_proposal_n20"
mkdir -p "$RESULT_DIR"
...
if [ -n "${DOUBLE_PROPOSAL_COUNTS_OVERRIDE:-}" ]; then
  read -r -a DOUBLE_PROPOSAL_COUNTS <<< "${DOUBLE_PROPOSAL_COUNTS_OVERRIDE}"
else
  DOUBLE_PROPOSAL_COUNTS=(0 1 3)
fi
...
run_single_experiment() {
  ...
  local result_file="$RESULT_DIR/TD-Hotstuff_doubleproposal${num_bad}.txt"

  echo ""
  echo "======================================================================"
  echo "  EXPERIMENT: DoubleProposal bad=$num_bad ids=${bad_node_ids:-none} n=$N"
  echo "======================================================================"
...
"
  ...
  export TD_HS_DOUBLE_PROPOSAL_START_VIEW="${TD_HS_DOUBLE_PROPOSAL_START_VIEW:-${TD_HS_STRONG_FAULT_ATTACK_START_VIEW}}"
  ...
  export TD_HS_WEIGHT_UPDATE_ACTIVATION_EPOCH_DELAY="${TD_HS_WEIGHT_UPDATE_ACTIVATION_EPOCH_DELAY:-1}"
  ...
  # Double proposal can split votes and prevent QC formation. Enable the
  # experiment pacemaker so replicas can leave equivocated views and reach
  # the certified penalty activation boundary.
  ...
  if [ "$num_bad" -gt 0 ]; then
    export TD_HS_DOUBLE_PROPOSAL_IDS="$bad_node_ids"
  else
    unset TD_HS_DOUBLE_PROPOSAL_IDS
  fi
  ...
  if [ -n "$log_files" ]; then
    ...
    local keep_dir="$RESULT_DIR/logs_TD-Hotstuff_doubleproposal${num_bad}"
    rm -rf "$keep_dir"
    mkdir -p "$keep_dir"
...
for num_bad in "${DOUBLE_PROPOSAL_COUNTS[@]}"; do
...
echo ""
echo "======================================================================"
echo "  DOUBLE-PROPOSAL EXPERIMENTS COMPLETE (n=$N)"
