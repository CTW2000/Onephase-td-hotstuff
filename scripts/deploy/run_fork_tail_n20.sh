#!/bin/bash
# Fork Tail experiment for n=20 using 6 machines (5 node servers + 1 client).
# Runs sequentially — NO parallel experiments.
# Protocols: HS-1, HS-2, HS, HS-1-SLOT | fork counts: 0, 1, 4
...
RESULT_DIR="$DEPLOY_DIR/experiment_results/tail_forking_n20"
mkdir -p "$RESULT_DIR"
...
echo ""
echo "######################################################################"
echo "# FORK TAIL EXPERIMENT | n=$N | timer=${TIMER}ms"
echo "######################################################################"
...
generate_config(config_path='./config/${cfg}.config',
                max_process_txn=$mpt, fork_tail_num=$num_fork, timer_length=$TIMER)
generate_performance_server_conf($N)
"
    # Verify fault param actually landed in config (catches silent permission/write failures)
    actual_fork=$(grep fork_tail_num ./config/${cfg}.config | grep -o '[0-9]\+')
    if [ "$actual_fork" != "$num_fork" ]; then
      echo "ERROR: ${cfg}.config has fork_tail_num=$actual_fork, expected $num_fork — ABORTING"
      exit 1
    fi

    result_file="$RESULT_DIR/${proto}_fork${num_fork}.txt"
    run_single_experiment "$proto" "TailForking fork=$num_fork timer=${TIMER}ms n=$N" "$result_file" "./config/performance.conf"
  done
done
...
echo ""
echo "======================================================================"
echo "  FORK TAIL EXPERIMENTS COMPLETE (n=$N)"
