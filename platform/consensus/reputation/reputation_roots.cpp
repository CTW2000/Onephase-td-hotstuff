...
std::vector<int> BuildLeaderEpochSchedule(
  ...
  if (total <= 0) {
    return BuildHistoryCooldownFairnessSchedule(
        effective, total_replicas, epoch_start_view, epoch_views);
  }

  bool exact_round_robin = true;
  int64_t first_weight = 0;
  for (int64_t weight : effective) {
    if (weight <= 0) {
      exact_round_robin = false;
      break;
    }
    if (first_weight == 0) {
      first_weight = weight;
    } else if (weight != first_weight) {
      exact_round_robin = false;
      break;
    }
  }

  const std::vector<int> cycle = BuildSmoothWeightedCycle(effective, total_replicas);
  std::vector<int> epoch;
  epoch.reserve(static_cast<size_t>(epoch_views));
  if (!exact_round_robin) {
    return BuildHistoryCooldownFairnessSchedule(
        effective, total_replicas, epoch_start_view, epoch_views);
  }
  for (int offset = 0; offset < epoch_views; ++offset) {
    int idx = PositiveModulo(epoch_start_view + offset,
