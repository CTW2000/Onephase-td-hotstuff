...
std::string AuditValidatorsJson(const std::vector<ValidatorReputation>& validators) {
  ...
  for (size_t i = 0; i < validators.size(); ++i) {
    ...
    out << "{\"id\":" << validator.validator_id
        ...
        << ",\"leader_dirichlet_commit\":"
        << validator.leader_dirichlet_commit
        << ",\"leader_dirichlet_certify_only\":"
        << validator.leader_dirichlet_certify_only
        << ",\"leader_dirichlet_timeout\":"
...
bool CandidateIsPreferredOver(const ReputationCandidate& lhs,
                              const ReputationCandidate& rhs) {
  return std::tie(lhs.end_view, lhs.start_view, lhs.activation_view,
                  lhs.candidate_digest_hex) >
         std::tie(rhs.end_view, rhs.start_view, rhs.activation_view,
...
bool ReputationPluginRuntime::RecordInvalidQcProposalEvidence(
    InvalidQcProposalEvidence evidence) {
  RuntimeEvent event;
  event.type = RuntimeEvent::Type::kInvalidQcProposalEvidence;
  event.invalid_qc_proposal = std::move(evidence);
  return Enqueue(std::move(event));
}
...
ReputationWeightSnapshot ReputationPluginRuntime::SnapshotForInvalidQcProposal(
...
void ReputationPluginRuntime::ProcessInvalidQcProposalEvidence(
    InvalidQcProposalEvidence evidence) {
  if (evidence.view_or_round < 0 || evidence.leader_id <= 0 ||
      evidence.protocol_id.empty() || evidence.proposal_hash.empty() ||
      !evidence.proposal_signature_verified || evidence.qc_verified) {
    return;
  }
  const uint64_t window_index =
      static_cast<uint64_t>(evidence.view_or_round) / options_.window_size_views;
  ...
  const int window_end =
      static_cast<int>(window_start + options_.window_size_views);
  ReputationWeightSnapshot snapshot = SnapshotForInvalidQcProposal(evidence);
  WindowKey key;
  key.window_index = window_index;
  key.weight_root_hex = snapshot.weight_root_hex;
  key.weight_version = snapshot.weight_version;
  ...
  const auto dedupe_key =
      std::make_tuple(evidence.protocol_id, evidence.weight_version,
                      evidence.leader_id, evidence.view_or_round,
                      evidence.slot_or_height, evidence.proposal_hash);
  if (!buffer.seen_invalid_qc_proposals.insert(dedupe_key).second) {
    return;
  }
  buffer.invalid_qc_proposals.push_back(std::move(evidence));
}
...
void ReputationPluginRuntime::FinalizeWindow(const WindowKey& key,
  ...
  leader_dirichlet_counters.reserve(candidate.validators.size());
  for (const ValidatorReputation& validator : candidate.validators) {
    ...
    LeaderDirichletCounter leader_counter;
    leader_counter.commit = validator.leader_dirichlet_commit;
    leader_counter.certify_only = validator.leader_dirichlet_certify_only;
    leader_counter.timeout = validator.leader_dirichlet_timeout;
    leader_dirichlet_counters.push_back(leader_counter);
  }
  ...
  if (!leader_dirichlet_counters.empty()) {
    const auto current_snapshot_key =
        std::make_pair(buffer->snapshot.weight_root_hex,
                       buffer->snapshot.weight_version);
    const auto next_snapshot_key =
        std::make_pair(candidate.next_weight_root_hex,
                       candidate.old_weight_version + 1);
    std::lock_guard<std::mutex> lk(mutex_);
    prior_leader_dirichlet_counters_[current_snapshot_key] =
        leader_dirichlet_counters;
    prior_leader_dirichlet_counters_[next_snapshot_key] =
        std::move(leader_dirichlet_counters);
  }
  if (options_.config.peertrust_enabled) {
...
void ReputationPluginRuntime::ApplyPersistentStrongFaults(
  ...
  for (int validator_id : persistent_faults) {
    ...
    if (validator_id <= static_cast<int>(candidate->leader_weights.size())) {
      candidate->leader_weights[validator_id - 1] = penalty_weight;
    }
  }
}
